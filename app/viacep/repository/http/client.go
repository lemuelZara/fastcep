...
func (c Client) GetByCEP(ctx context.Context, cep string) (app.Address, error) {
	...
	path := fmt.Sprintf("http://viacep.com.br/ws/%s/json/", cep)
	...
	if err != nil {
		return app.Address{}, fmt.Errorf(`failed on getting cep: %w\n`, err)
	}
...
func parseResponse(resp *http.Response) (app.Address, error) {
	...
	if err != nil {
		return app.Address{}, fmt.Errorf("parse address failed: %w\n", err)
	}
